...
namespace Datos
{
    public class puntoventaDL
    {
        ...
        {
            using (IDataReader datareader = conexion.executeOperation("fn_puntodeventa_listar",
                CommandType.StoredProcedure))
            {
                List<puntoventa> listado = new List<puntoventa>();
                while (datareader.Read())
                {
                    puntoventa registro = new puntoventa();
                    registro.idpuntoventa = Convert.ToInt32(datareader["p_inidpuntoventa"]);
                    registro.nombrepuntaventa = Convert.ToString(datareader["chnombrepuntoventa"]);
